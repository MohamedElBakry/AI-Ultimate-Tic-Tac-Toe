...
class Agent {
    ...
    constructor(state, type, piece) {
        ...
        this.type = type;
        this.piece = piece;
        this.opponentPiece = getNext(this.piece);
    }
    ...
    playOptimalMove(depth, state) {
      ...
      if (!state)
        ...
        state.turn = this.opponentPiece;   // TODO:  change this to be dynamically chosen -- i.e. the opposite of this.piece
    ...
    evaluate(state) {
      ...
      miniMaxAlphaBetaPruning(depth, state, alpha, beta, isMaxing) {
        ...
        if (isMaxing) {
            ...
            let score = this.miniMaxAlphaBetaPruning(depth - 1, state, alpha, beta, false);
            ...
            state.previousMove = move;
            // Evaluate
            let score = this.miniMaxAlphaBetaPruning(depth - 1, state, alpha, beta, true);
            // Undo
            // state.previousMove = truePreviousMove;
            state.board[move.x][move.y] = game.none;
