...
class Agent {
    ...
    constructor(state, type, piece=Agent.piece.X, isWorker=false) {
        ...
        if (!isWorker)
            this.workers = [...Array(navigator.hardwareConcurrency)].map(_ => new Worker("ai-worker.js"));
    }
    ...
    async getScoresFromThreads(workBatches, numWorkers, workers) {
        const results = [];
